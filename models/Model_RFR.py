...
def main():
    ...
    train = pd.DataFrame.from_records(
        json.load(open("poc_train_eda_lan_j (1).json"))
    ).fillna("")
    test = pd.DataFrame.from_records(
        json.load(open("poc_test_eda_lan_j (1).json"))
    ).fillna("")
    ...
    err_random_forest = mean_absolute_error(
        val["year"].values, random_forest.predict(val.drop("year", axis=1))
    )
...
# Finetune model including preprocess features
...
def main():
    ...
    logging.info("Loading training/test data")
    train = pd.DataFrame.from_records(
        json.load(open("poc_feature_train_eda_lan_j.json"))
    ).fillna("")
    test = pd.DataFrame.from_records(
        json.load(open("poc_feature_test_eda_lan_j.json"))
    ).fillna("")
    logging.info("Splitting validation")
    train, val = train_test_split(train, stratify=train["year"], random_state=123)
    featurizer = ColumnTransformer(
        transformers=[
            ("title_pre", TfidfVectorizer(max_features=6000), "title_pre"),
            ("abstract_pre", TfidfVectorizer(max_features=6000), "abstract_pre"),
            ("publisher_low", TfidfVectorizer(max_features=6000), "publisher_low"),
        ],
        remainder="drop",
    )
...
# INFO:root:Fitting models
# INFO:root:Evaluating on validation data
# INFO:root:Random Forest regressor MAE: 3.295227122880652
# INFO:root:Predicting on test
# INFO:root:Writing prediction file
